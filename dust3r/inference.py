...
def transpose_list(x: list[dict]) -> dict[list]:
...
def loss_of_one_batch(
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            loss = (
                criterion(view_ref, views_source, pred_ref, preds_source)
    ...
    result = dict(
        view_ref=view_ref,
        views_source=transpose_list(views_source),
        pred_ref=pred_ref,
        preds_source=transpose_list(preds_source),
        loss=loss,
    )
...
@torch.no_grad()
def inference(stars, model, device, batch_size=8, verbose=True):
    if verbose:
        print(f">> Inference with model on {len(stars)} image stars")
    result = []
    ...
    for i in tqdm.trange(0, len(stars), batch_size, disable=not verbose):
        res = loss_of_one_batch(
            collate_with_cat(stars[i : i + batch_size]), model, None, device
        )
        result.append(to_cpu(res))

    result = collate_with_cat(
        result, lists=multiple_shapes, dont_collate_nested_lists=True
    )
